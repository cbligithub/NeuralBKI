...
class KittiDataset(Dataset):
    ...
    def __init__(self,
        ...
        for seq in self._seqs:
            ...
            self._velodyne_list.extend([os.path.join(velodyne_dir, str(frame).zfill(6) + '.bin')
        ...
        assert len(self._velodyne_list) == np.sum(
            self._num_frames_scene), f"inconsitent number of frames detected, check the dataset"
